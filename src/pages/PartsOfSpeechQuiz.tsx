...
import quizDataRaw from "../data/parts-of-speech/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "基礎辨識", emoji: "🔵", desc: "基礎詞性辨識" },
  { key: "topic2", label: "對比判斷", emoji: "🟢", desc: "詞性對比判斷" },
  { key: "topic3", label: "綜合挑戰", emoji: "🔴", desc: "綜合題＋找出正確詞性" },
];
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("pos-s1"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("pos-s2"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("pos-s3"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 詞性大冒險 — 考試設定</h1>
