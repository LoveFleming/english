...
import quizDataRaw from "../data/classical-chinese/quiz_questions.json";
...
type FilterMode = "new" | "failed" | "all" | "s1" | "s2" | "s3" | "s4" | "s5" | "s6";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "all", label: "全部 30 題", emoji: "📚", desc: "所有題目隨機出" },
  { key: "s1", label: "於", emoji: "🔵", desc: "6 題「於」字比較" },
  { key: "s2", label: "之", emoji: "🟢", desc: "6 題「之」字比較" },
  { key: "s3", label: "其", emoji: "🟣", desc: "5 題「其」字比較" },
  { key: "s4", label: "以", emoji: "🟠", desc: "5 題「以」字比較" },
  { key: "s5", label: "而", emoji: "🔴", desc: "4 題「而」字比較" },
  { key: "s6", label: "乃·且", emoji: "🔵", desc: "4 題「乃」「且」比較" },
];

export default function ClassicalChineseQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "s1":
        return allQuestions.filter(q => q.id.startsWith("cc-s1"));
      case "s2":
        return allQuestions.filter(q => q.id.startsWith("cc-s2"));
      case "s3":
        return allQuestions.filter(q => q.id.startsWith("cc-s3"));
      case "s4":
        return allQuestions.filter(q => q.id.startsWith("cc-s4"));
      case "s5":
        return allQuestions.filter(q => q.id.startsWith("cc-s5"));
      case "s6":
        return allQuestions.filter(q => q.id.startsWith("cc-s6"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 文言文字義比較 — 考試設定</h1>
  ...
  if (phase === "active") {
    ...
    const topicPrefix = q.id.split("-").slice(0, 2).join("-");
    ...
    return (
      <div className="space-y-4 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        ...
        <Card className="p-6 space-y-4">
          ...
          <h2 className="text-lg font-bold text-stone-900 leading-relaxed">{q.question}</h2>
        ...
        <Card className="p-6 space-y-4">
          ...
          {wrongResults.map((r, i) => (
            <div key={i} className="p-4 bg-red-50 rounded-xl border border-red-200 space-y-2">
              <p className="font-bold text-stone-900 text-sm">{r.question}</p>
              <p className="text-red-600 text-sm">你答：<strong>{r.userAnswer}</strong></p>
              <p className="text-green-600 text-sm">正確：<strong>{r.correctAnswer}</strong></p>
              <p className="text-sm text-zinc-600 bg-white p-3 rounded-lg">💡 {r.explanation}</p>
            </div>
          ))}
        </Card>
      )}
        ...
        <Card className="p-6 space-y-3">
          ...
          {correctResults.map((r, i) => (
            <div key={i} className="p-3 bg-green-50 rounded-lg text-sm">
              <span className="text-green-600 font-bold">✓</span> {r.question.substring(0, 40)}... → <strong>{r.correctAnswer.substring(0, 30)}...</strong>
            </div>
          ))}
        </Card>
      )}
