...
import quizDataRaw from "../data/past-tense/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "過去式選擇", emoji: "🔵", desc: "動詞原形→過去式" },
  { key: "topic2", label: "填空題", emoji: "🟢", desc: "填入正確的過去式" },
  { key: "topic3", label: "是非題", emoji: "🟡", desc: "判斷用法正確與否" },
  { key: "topic4", label: "改錯/綜合", emoji: "🔴", desc: "改錯題 + 綜合應用" },
];

export default function PastTenseQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "topic4":
        return allQuestions.filter(q => q.id.startsWith("q4") || q.id.startsWith("q5"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 動詞過去式 — 考試設定</h1>
