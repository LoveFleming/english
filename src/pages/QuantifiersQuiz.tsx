...
import quizDataRaw from "../data/quantifiers/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "選擇題", emoji: "🔵", desc: "many/much/few/little..." },
  { key: "topic2", label: "填空概念", emoji: "🟢", desc: "填入正確的數量形容詞" },
  { key: "topic3", label: "是非題", emoji: "🟡", desc: "判斷用法正確與否" },
  { key: "topic4", label: "改錯題", emoji: "🔴", desc: "找出並修正錯誤" },
];

export default function QuantifiersQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("q3") || q.id.startsWith("q4"));
      case "topic4":
        return allQuestions.filter(q => q.id.startsWith("q5") || q.id.startsWith("q6"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 數量形容詞 — 考試設定</h1>
  ...
  if (phase === "active") {
    ...
    const topicPrefix = q.id.split("-")[0].split(/\d/)[0] + q.id.match(/-\d+/)?.[0]?.replace(/-\d+/, (m: string) => q.id.charAt(1)) || "";
