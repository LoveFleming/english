...
export default function PrepositionalPhrasesQuiz() {
  ...
  const submitQuiz = () => {
    const graded: GradedItem[] = items.map(item => {
      ...
      return { id: item.id, question: item.question, userAnswer, correctAnswer: item.correctAnswer, explanation: item.explanation, isCorrect: userAnswer === item.correctAnswer };
    });
    ...
    if (isAuthenticated && user) {
      ...
      const examScore: ExamScore = { id: Date.now().toString(), date: new Date().toISOString(), subject: "介系詞片語", score: scorePct, totalQuestions: items.length, correctAnswers: correctCount, wrongAnswers: wrongQuestions.length, wrongQuestions };
  ...
  if (phase === "setup") {
    return (
      <div className="max-w-2xl mx-auto py-6 px-2">
        <h1 className="text-2xl sm:text-3xl font-bold text-stone-900 mb-2">📝 介系詞片語 Quiz</h1>
          ...
          <div className="flex flex-col items-center py-6 space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-4 w-full">
              {failedItems.length > 0 && (
                <button onClick={() => { const fq = items.filter(i => failedItems.some(f => f.id === i.id)); setItems(fq); setCurrentIndex(0); setAnswers({}); setPhase("active"); }} className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl transition-all flex-1">🔄 重考錯題 ({failedItems.length})</button>
              )}
              <button onClick={() => setPhase("setup")} className="px-5 py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold rounded-xl border border-stone-300 transition-all flex-1">↩ 回到選單</button>
            </div>
          </div>
        </Card>
        {failedItems.length > 0 && (
    ...
    <div className="max-w-2xl mx-auto py-6 px-2">
      ...
      <Card className="border-blue-200 shadow-lg !rounded-2xl overflow-hidden">
        <div className="bg-blue-50/50 -mx-4 -mt-4 mb-5 px-5 py-6 border-b border-blue-100">
          ...
          <h2 className="text-xl sm:text-2xl font-medium text-stone-900 leading-relaxed">{currentItem.question}</h2>
        </div>
      ...
      <div className="mt-6 text-center">
        <button onClick={() => setPhase("setup")} className="text-stone-400 text-sm hover:text-stone-600 transition-colors">取消測驗</button>
      </div>
    </div>
  );
}
