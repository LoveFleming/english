...
import quizDataRaw from "../data/measure_words/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "a few/a little", emoji: "🔵", desc: "可數與不可數的選擇" },
  { key: "topic2", label: "計量單位", emoji: "🟢", desc: "選出正確的計量單位" },
  { key: "topic3", label: "是非+改錯", emoji: "🟡", desc: "判斷正確與找出錯誤" },
  { key: "topic4", label: "綜合+進階", emoji: "🟣", desc: "綜合應用與進階挑戰" },
];

export default function MeasureWordsQuiz() {
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 計量單位 — 考試設定</h1>
