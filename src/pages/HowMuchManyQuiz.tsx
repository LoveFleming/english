...
import quizDataRaw from "../data/how_much_many/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "all", label: "全部", emoji: "📚", desc: "所有50題隨機出" },
  { key: "topic1", label: "How many are there", emoji: "🔵", desc: "How many + are there 句型" },
  { key: "topic2", label: "How many 助動詞", emoji: "🟢", desc: "How many + 助動詞句型" },
  { key: "topic3", label: "How much", emoji: "🟡", desc: "How much 句型練習" },
  { key: "topic4", label: "Which + 綜合", emoji: "🟣", desc: "Which 選擇 + 改錯 + 綜合" },
];

export default function HowMuchManyQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("q1"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("q2"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("q3"));
      case "topic4":
        return allQuestions.filter(q => q.id.startsWith("q4") || q.id.startsWith("q5") || q.id.startsWith("q6"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 How Much / How Many — 考試設定</h1>
  ...
  if (phase === "active") {
    ...
    const topicKey = q.id.substring(0, 2);
