...
import quizDataRaw from "../data/auxiliary-be/quiz_questions.json";
...
const TOPIC_LABELS: Record<string, string> = {
  be: "🔵 Be 動詞 (am/is/are/was/were)",
  aux: "🟢 助動詞 (can/will/should/must/do/does/did)",
  ans: "🟡 問句的回答",
  hav: "🔴 have to + 原形",
};
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "Be 動詞", emoji: "🔵", desc: "am/is/are/was/were" },
  { key: "topic2", label: "助動詞", emoji: "🟢", desc: "can/will/should/must..." },
  { key: "topic3", label: "問句回答", emoji: "🟡", desc: "用什麼問就用什麼答" },
  { key: "topic4", label: "have to", emoji: "🔴", desc: "have to / has to + 原形" },
];

export default function AuxiliaryBeVerbsQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("be"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("aux"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("ans"));
      case "topic4":
        return allQuestions.filter(q => q.id.startsWith("hav"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 助動詞 & Be 動詞 — 考試設定</h1>
