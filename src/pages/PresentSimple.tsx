...
import markdownContent from "../data/simple_sentence/present_simple.md?raw";

export default function PresentSimple({ openApp }: { openApp: (id: string) => void }) {
  return (
    <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
      <div className="flex items-center justify-between mb-4 pb-4 border-b border-zinc-200">
        <h1 className="text-3xl font-bold text-stone-900 tracking-tight">現在簡單式 (Present Simple Tense)</h1>
        <button
          onClick={() => openApp("quiz.present-simple")}
