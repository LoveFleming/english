...
import quizDataRaw from "../data/classical-particles/quiz_questions.json";
...
type FilterMode = "new" | "failed" | "all" | "topic1" | "topic2" | "topic3";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "基礎虛字辨識", emoji: "🔵", desc: "單句虛字意思判斷" },
  { key: "topic2", label: "一字多義對比", emoji: "🟢", desc: "同字不同義比較" },
  { key: "topic3", label: "會考綜合挑戰", emoji: "🔴", desc: "段落理解＋多字比較" },
];

export default function ClassicalParticlesQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("cp-s1"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("cp-s2"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("cp-s3"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 文言虛字基本功 — 考試設定</h1>
    ...
    return (
      <div className="space-y-4 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        ...
        <Card className="p-6 space-y-4">
          ...
          <h2 className="text-xl font-bold text-stone-900 whitespace-pre-line">{q.question}</h2>
        ...
        <Card className="p-6 space-y-4">
          ...
          {wrongResults.map((r, i) => (
            <div key={i} className="p-4 bg-red-50 rounded-xl border border-red-200 space-y-2">
              <p className="font-bold text-stone-900 whitespace-pre-line">{r.question}</p>
