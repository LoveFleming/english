...
import quizDataRaw from "../data/compound-words/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "基礎辨識", emoji: "🔵", desc: "基礎複詞分類" },
  { key: "topic2", label: "進階判斷", emoji: "🟢", desc: "更多詞語判斷" },
  { key: "topic3", label: "綜合挑戰", emoji: "🔴", desc: "混合題＋找出正確類型" },
];

export default function CompoundWordsQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("cw-s1"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("cw-s2"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("cw-s3"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 複詞三兄弟 — 考試設定</h1>
