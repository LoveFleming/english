...
import quizDataRaw from "../data/word-meanings/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "topic1", label: "基礎一字多義", emoji: "🔵", desc: "找意思相同的配對" },
  { key: "topic2", label: "找不同的意思", emoji: "🟢", desc: "找出與眾不同的意思" },
  { key: "topic3", label: "會考綜合挑戰", emoji: "🔴", desc: "文言文語境一字多義判斷" },
];

export default function WordMeaningsQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("wm-s1"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("wm-s2"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("wm-s3"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 一字多義達人 — 考試設定</h1>
