...
import quizDataRaw from "../data/irregular-verbs/quiz_questions.json";
...
const FILTER_OPTIONS: { key: FilterMode; label: string; emoji: string; desc: string }[] = [
  ...
  { key: "all", label: "全部", emoji: "📚", desc: "所有 50 題隨機出" },
  { key: "topic1", label: "ABC 型", emoji: "🔵", desc: "go-went-gone 型" },
  { key: "topic2", label: "ABB 型", emoji: "🟢", desc: "make-made-made 型" },
  { key: "topic3", label: "AAA/ABA 型", emoji: "🟡", desc: "put-put-put / come-came-come" },
  { key: "topic4", label: "應用題", emoji: "🔴", desc: "句子填空" },
];
...
export default function IrregularVerbsQuiz() {
  ...
  const getFilteredQuestions = (mode: FilterMode): QuizItem[] => {
    switch (mode) {
      ...
      case "all":
        return [...allQuestions];
      case "topic1":
        return allQuestions.filter(q => q.id.startsWith("v01"));
      case "topic2":
        return allQuestions.filter(q => q.id.startsWith("v02"));
      case "topic3":
        return allQuestions.filter(q => q.id.startsWith("v03"));
      case "topic4":
        return allQuestions.filter(q => q.id.startsWith("v04"));
      default:
        return [...allQuestions];
    }
  };
  ...
  if (phase === "setup") {
    ...
    return (
      <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
        <h1 className="text-2xl font-bold text-stone-900">📝 不規則動詞 — 考試設定</h1>
        <Card className="p-6 space-y-4">
          ...
          <div className="flex items-center justify-between pt-4 border-t">
            <span className="text-sm text-zinc-500">可用題目：{available} 題</span>
            <button
              onClick={startQuiz}
  ...
  if (phase === "active") {
    ...
    const topicKey = q.id.substring(0, 3);
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in slide-in-from-bottom-2 duration-500">
      <Card className="p-8 text-center space-y-4">
        ...
        <h2 className="text-3xl font-bold text-stone-900">{score} / {items.length}</h2>
        <p className="text-xl text-zinc-600">正確率 {pct}%</p>
        <p className="text-sm text-zinc-500">
          {pct >= 90 ? "太厲害了！不規則動詞大師！🏆" :
           pct >= 80 ? "很不錯！再多背幾個就完美了 ✨" :
           pct >= 60 ? "有進步！多練幾次會更好 💪" :
           "別氣餒！回去看表格再背一次 📖"}
        </p>
